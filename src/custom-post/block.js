...
registerBlockType( 'cgb/gostudy-custom-post', {
	...
	edit({ attributes, className, setAttributes, focus }) {
		...
		const setBackgroundColor = (value) => {
			setAttributes({bannerBgColor : value || null });
			//console.log(value)
		};
		...
		const triggerChange = () => {
			...
			var elements = document.getElementsByClassName('post-type-select');
		...
		let classes='gostudy-related-posts__dynamic';
		let classesC='gostudy-related-posts container';
		if(attributes.imageUrl){
		...
		return (
			<Fragment>
				<InspectorControls>
					<PanelBody title={ __( 'Choose a post type' ) }>
						{ __( 'Please manually reselect every field after changing posttype or taxonomy' ) }<br/><br/>
						<SelectControl
								...
								onChange={ content => setAttributes({postType : content || null}) }
						...
						<SelectControl
								...
								options={ JSON.parse( gostudyAdminData.types_taxonomy )[attributes.postType] }
								onChange={ content => setAttributes({taxonomy : content || null }) }
						...
						<SelectControl
								...
								options={ JSON.parse( gostudyAdminData.terms )[[attributes.taxonomy]] }
								onChange={ content => setAttributes({term : content || null }) }
						...
						<SelectControl
								...
								label="Relationship"
								help=""
								value={ attributes.relationship }
								options={ [ { label: 'OR', value: 'OR' },{ label: 'AND', value: 'AND' }] }
								onChange={ content => setAttributes({relationship: content || null }) }
						...
						<SelectControl
								...
								label="Taxonomy"
								help=""
								value={ attributes.taxonomy2 }
								options={ JSON.parse( gostudyAdminData.types_taxonomy )[attributes.postType] }
								onChange={ content => setAttributes({taxonomy2 : content || null }) }
						...
						<SelectControl
								...
								label="Term"
								help=""
								value={ attributes.term2 }
								options={ JSON.parse( gostudyAdminData.terms )[[attributes.taxonomy2]] }
								onChange={ content => setAttributes({term2 : content || null }) }
			...
			<div className={classesC}>
				<h5 className="text-light-gray">Related posts</h5>
				<span className="related-posts__title">
					<PlainText
						onChange={ content => setAttributes({ title: content || null }) }
						value={ attributes.title }
						placeholder="Related posts title"
						className="heading"
					/>
				</span>
				<RichText
					...
					multiline="p"
					placeholder="Related posts text"
				/>
				<div className="related-posts__btn">
					<PlainText
